...
namespace AutoDuty.Helpers
{
    internal static class GotoBarracksHelper
    {
        ...
        internal static void GotoBarracksUpdate(IFramework framework)
        {
            ...
            if (Svc.ClientState.TerritoryType != ObjectHelper.GrandCompanyTerritoryType(ObjectHelper.GrandCompany) || _barracksDoorGameObject == null || Vector3.Distance(Svc.ClientState.LocalPlayer.Position, _barracksDoorGameObject.Position) > 3f)
            {
                GotoHelper.Invoke(ObjectHelper.GrandCompanyTerritoryType(ObjectHelper.GrandCompany), [_barracksDoorLocation], 0.25f, 3f);
                return;
            }
